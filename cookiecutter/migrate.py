...
def main() -> None:
    ...
    print("=" * 72)
    print("Updating the release instructions in CONTRIBUTING.md...")
    migrate_contributing_release_instructions()
    print("=" * 72)
    print()
...
"""Replacements bringing ``CONTRIBUTING.md`` up to date with the template.

Each entry is an ``(old, new)`` pair.  A pair whose ``new`` is already
present in the file has already been applied and is skipped, which is what
makes this step idempotent for the entries that only *insert* text (where
``old`` still matches after the insertion).
"""


def migrate_contributing_release_instructions() -> None:
    """Update the release instructions in ``CONTRIBUTING.md``.

    The template now explains that the release notes are read from the
    ``RELEASE_NOTES.md`` committed at the tagged commit (so they can't be
    cleaned up afterwards) and that the tag signature is enforced by the
    *Protect released tags* ruleset, which also makes released tags
    immutable.  It also fixes two typos.

    Only the exact paragraphs generated by the template are rewritten.
    Projects that customized them are left alone, as re-flowing arbitrary
    prose is not something this script can do safely.
    """
    contributing = Path("CONTRIBUTING.md")
    if not contributing.exists():
        print(f"  Skipped {contributing}: file not found")
        return
    ...
    try:
        ...
        manual_step(
            f"Failed to read {contributing}: {exc}. Please update the "
            "`Releasing` section from the template manually."
        )
        return

    new_text = text
    missing = 0
    for old, new in _CONTRIBUTING_RELEASE_UPDATES:
        if new in new_text:
            continue
    ...
    if missing:
        manual_step(
            f"{contributing} doesn't match the template in {missing} place(s), "
            "so it was not fully updated. Please copy the `Releasing` section "
            "from the template manually."
        )

    if new_text == text:
        if not missing:
            print(f"  Skipped {contributing}: release instructions already up to date")
        return

    try:
        replace_file_atomically(contributing, new_text)
        print(f"  Updated {contributing}: refreshed the release instructions")
    except OSError as exc:
        manual_step(
            f"Failed to update {contributing}: {exc}. Please update the "
            "`Releasing` section from the template manually."
        )
