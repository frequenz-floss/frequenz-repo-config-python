...
needs a few extra steps around that procedure.
...
1. Bump the `frequenz-repo-config` version used by the cookiecutter template to
   ...
   Remember to regenerate golden tests afterwards as described in [Update the
   golden test fixtures](#2-update-the-golden-test-fixtures) above.
...
For a patch release branched from a maintenance branch (`vX.Y.x`), skip the
